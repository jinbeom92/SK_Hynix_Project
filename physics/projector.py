# =================================================================================================
# Projector Modules for Parallel-Beam 3D CT
# -------------------------------------------------------------------------------------------------
# Purpose
#   Provides differentiable forward projection (FP) and backprojection (BP) operators for
#   parallel-beam 3D computed tomography. These operators define the physical mapping
#   between voxel volumes and detector-domain sinograms.
#
# Classes
#   • BaseProjector3D
#       Abstract base defining FP/BP interfaces and geometry reset.
#
#   • JosephProjector3D
#       Voxel-driven projector/backprojector using Joseph’s method:
#         - Uses trilinear interpolation (`grid_sample`) to accumulate ray integrals.
#         - Angle-, channel-, and step-chunking to control peak memory and streaming load.
#         - Forward: vol [B,C,D,H,W] → sino [B,C,A,V,U].
#         - Backproject: sino [B,C,A,V,U] → vol [B,C,D,H,W].
#         - Geometry reset updates trig caches and detector coordinates.
#
#   • SiddonProjector3D
#       Ray-driven projector/backprojector using Siddon’s analytic method:
#         - Traces rays voxel by voxel with analytic intersection lengths.
#         - More accurate but significantly slower (kept for validation/benchmarking).
#         - Forward/Backproject loops over all detector elements and angles.
#
# Functions
#   • make_projector(method, geom)
#       Factory to construct either JosephProjector3D or SiddonProjector3D.
#
# Memory/Performance Notes
#   • JosephProjector3D supports configurable `angle_chunk`, `c_chunk`, and `step_chunk`
#     to reduce memory usage during projection.
#   • Temporary tensors are explicitly deleted within loops to reduce fragmentation.
#   • SiddonProjector3D is CPU/GPU compatible but intended only for small volumes/tests.
#
# Usage
#   geom = Parallel3DGeometry(...)
#   proj = make_projector("joseph3d", geom)
#   sino = proj(vol)               # Forward projection
#   vol_bp = proj.backproject(sino)  # Backprojection
# =================================================================================================
...
class JosephProjector3D(BaseProjector3D):
    def __init__(self, geom: Parallel3DGeometry, n_steps: Optional[int] = None):
        ...
        # voxel spacing & integration setup
        ...
        for a0 in range(0, A, self.geom.angle_chunk):
            ...
            for k in range(D):
                ...
                for c0 in range(0, C, max(1, self.c_chunk)):
                    ...
                    samp = F.grid_sample(x_in, grid_cast, mode="bilinear", padding_mode="zeros", align_corners=True)
...
class SiddonProjector3D(BaseProjector3D):
    ...
    def _trace_ray(self, D, H, W, x0, y0, z0, cos_t, sin_t, device):
        ...
        def _t_for(p0, dp, pmin, pmax):
            if abs(dp) < 1e-12:
                if p0 < pmin or p0 > pmax: return float("inf"), float("-inf")
